...
class Enemy:
    ...
    '''def shoot(self):
        ...
        # Tính góc giữa enemy và người chơi
        angle = math.atan2(py - self.rect.y, px - self.rect.x)
        dx = math.cos(angle) * self.bullet_speed
        dy = math.sin(angle) * self.bullet_speed

        # Tạo viên đạn và thêm vào danh sách
        bullet = {'x': self.rect.centerx, 'y': self.rect.centery, 'dx': dx, 'dy': dy}
    ...
    def hit_by(self, bullet_x, bullet_y):
        # Kiểm tra viên đạn của người chơi có trúng enemy không
        if abs(bullet_x - self.rect.centerx) < 30 and abs(bullet_y - self.rect.centery) < 30:
            self.health -= 1  # Trúng đạn thì giảm máu
            return True
        return False

    def is_dead(self):
        # Trả về True nếu máu ≤ 0
